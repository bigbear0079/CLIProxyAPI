...
func TestFinishReasonToolCallsNotOverwritten(t *testing.T) {
	...
	fr1 := testjson.Get(result1[0], "choices.0.finish_reason")
	...
	fr2 := testjson.Get(result2[0], "choices.0.finish_reason").String()
	if fr2 != "tool_calls" {
		t.Errorf("Expected finish_reason 'tool_calls', got: %s", fr2)
	}

	// Verify native_finish_reason is lowercase upstream value
	nfr2 := testjson.Get(result2[0], "choices.0.native_finish_reason").String()
	if nfr2 != "stop" {
...
func TestFinishReasonStopForNormalText(t *testing.T) {
	...
	fr := testjson.Get(result2[0], "choices.0.finish_reason").String()
	if fr != "stop" {
...
func TestNoFinishReasonOnIntermediateChunks(t *testing.T) {
	...
	// Verify no finish_reason on intermediate chunk
	fr2 := testjson.Get(result2[0], "choices.0.finish_reason")
